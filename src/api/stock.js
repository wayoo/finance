...
class USStock extends Stock {
  ...
  getPrice() {
    ...
    return {
      name: record[0],
      close: record[1],
      price_change: record[4],
      price_change_percent: record[2],
      volume: record[10],
      amount: null,
      share: record[19],
      market_value: record[12],
      open: record[5],
      high: record[6],
      low: record[7],
...
class CNStock extends Stock {
  constructor(code) {
    super(code);
    if (this.code[0] === '6') {
      this.name = `s_sh${this.code}`;
    } else {
      this.name = `s_sz${this.code}`;
    }
  }
  ...
  getPrice() {
    ...
    return {
      name: record[0],
      close: record[1],
      price_change: record[2],
      price_change_percent: record[3],
      volume: record[4],
      amount: record[5] * 10000,
    };
  }
}
...
class StockFactory {
  constructor(c) {
    ...
    if (code.length === 6 && /^\d+$/.test(code)) {
      // china mainland stock
      this.instance = new CNStock(c);
      this.type = 'CN';
    } else if (/^([a-z]|[A-Z]|\$)+$/.test(code)) {
      // US market stock
      this.instance = new USStock(c);
      this.type = 'US';
    } else if (/^\d{4}$/.test(code)) {
      // HK market stock
      this.instance = new HKStock(c);
      this.type = 'HK';
    } else {
      console.error(`StockFactory can't handle: ${code}`);
    }
  }
...
function price(stockList) {
  ...
  return new Promise(((resolve) => {
    ...
    script.onload = function () {
      ...
      //   const record = window[`hq_str_${classify(item)}`].split(',');
      //   ret.push({
      //     name: record[0],
      //     close: record[1],
      //     price_change: record[2],
